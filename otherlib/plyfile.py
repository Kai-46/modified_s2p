...
class PlyData(object):
    ...
    def __init__(self, elements=[], text=False, text_fmt='%.18g', byte_order='=',
                 comments=[], obj_info=[]):
        '''
    ...
    def _parse_header(stream):
        ...
        return PlyData(PlyElement._parse_multi(lines[a:]),
                       ...
                       comments=comments['comment'], obj_info=comments['obj_info'])
    ...
    def write(self, stream):
        ...
        try:
            ...
            for elt in self:
                elt._write(stream, self.text, self.text_fmt, self.byte_order)
...
class PlyElement(object):
    ...
    def _write(self, stream, text, text_fmt, byte_order):
        '''
        Write the data to a PLY file.

        '''
        if text:
            self._write_txt(stream, text_fmt)
        else:
            if self._have_list:
    ...
    def _write_txt(self, stream, text_fmt):
        '''
        ...
        for rec in self.data:
            ...
            # _np.savetxt(stream, [fields], '%.4f', newline='\n')
            _np.savetxt(stream, [fields], text_fmt, newline='\n')
